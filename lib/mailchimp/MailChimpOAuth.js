...
MailChimpOAuth.prototype.spawnServer = function () {
  ...
  var callback = function (request, response) {
    ...
    if (request.method != 'GET') {
      self.emit('error', {err: new Error('Received something other than a GET request.'), data:undefined});
    ...
    if (typeof query.code !== 'undefined') {
      ...
      self.emit('error', {err: new Error('Received a request without a code.'), data:query});
      response.writeHead(500, {'Content-Type': 'text/plain'});
      response.end();
    }
  };
...
MailChimpOAuth.prototype.handleOAuthResponse = function (customParams) {
  ...
  if (typeof customParams.code !== 'undefined') {
    ...
    self.emit('error', {err: new Error('Received a request without a code.'), data:customParams});
  }
};
...
MailChimpOAuth.prototype.getAccessToken = function (customParams) {
  ...
  if (!customParams.code) {
    self.emit('error', {err: new Error('Code is required in Params'), data:customParams});
    return;
  }
  var params = {
  ...
  request({
    ...
    if (error) {
      self.emit('error', {err: new Error('Unable to connect to the MailChimp OAuth service.'), data:customParams});
    } else {
      ...
      try {
        ...
        self.emit('error', {err: new Error('Error parsing JSON answer from the MailChimp getAccessToken API.'), data:customParams});
        return;
      }
      ...
      if (typeof parsedResponse.access_token !== 'undefined') {
        ...
        self.emit('error', {err: new Error('Answer from MailChimp API does not contain an access token.'), data:customParams});
      }
...
MailChimpOAuth.prototype.getMetadata = function (customParams) {
  ...
  if (!customParams.accessToken) {
    self.emit('error', {err: new Error('accessToken is required in Params'), data:customParams});
    return;
  }
  request({
    ...
    if (error) {
      ...
      try {
        ...
        self.emit('error', {err: new Error('Error parsing JSON answer from the MailChimp getMetadata API.'), data:customParams});
        return;
      }

      if (typeof parsedResponse.dc !== 'undefined') {
        customParams.metadata = parsedResponse;
        customParams.apiKey = customParams.accessToken + '-' + customParams.dc;
        self.emit('receivedMetadata', customParams);
      } else {
        self.emit('error', {err: new Error('Answer from MailChimp API does not contain a datacenter pointer.'), data:customParams});
      }
