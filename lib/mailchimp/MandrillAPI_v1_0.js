...
MandrillAPI_v1_0.prototype.execute = function (method, availableParams, givenParams, callback) {
	...
	request({
		...
		var parsedResponse;
		if (error) {
			callback({ 'error' : 'Unable to connect to the Mandrill API endpoint.', 'code' : 'xxx' });
		} else {
			try {
				parsedResponse = JSON.parse(body); 
			} catch (error) {
				callback({ 'error' : 'Error parsing JSON answer from Mandrill API.', 'code' : 'xxx' });
				return;
			}
...
MandrillAPI_v1_0.prototype.call = function (category, method, params, callback) {
	...
	if (typeof category != 'string' || typeof method != 'string') {
		callback({ 'error' : 'You have to provide the category and the name of the method to call.', 'code' : 'xxx' });
		return;
	}
	...
	if (typeof this[methodToCall] == 'function') {
		...
		callback({ 'error' : 'The API method '+methodToCall+' does not exist.', 'code' : 'xxx' });
		return;
	}
