...
function MailChimpOAuth(options) {
  ...
  try {
    ...
    self.emit('error', new Error('Error while getting token'), error);
  }
  ...
  request({
    ...
    if (error) {
      ...
      try {
        ...
        console.log('Error parsing JSON answer from the MailChimp API.', body);
        self.emit('error', new Error('Error parsing JSON answer from the MailChimp API.', customParams));
        return;
      }
  ...
  request({
    ...
    if (error) {
      ...
      try {
        ...
        console.error('Error parsing JSON answer from the MailChimp API.', body);
        self.emit('error', new Error('Error parsing JSON answer from the MailChimp API.'), customParams);
        return;
      }
